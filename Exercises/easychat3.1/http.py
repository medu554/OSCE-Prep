...
def main():
    ...
    with s_block("Request-Line"):
        ...
        s_string("/chat.ghp?username=", name='Request-URI', fuzzable=False)
        s_string("a", name='Request-URIa')
        s_string("&password=", name='Request-URIb', fuzzable=False)
        s_string("b", name='Request-URIc')
        s_string("&room=", name='Request-URId', fuzzable=False)
        s_string("c", name='Request-URIe')
        s_string("&sex=", name='Request-URIf', fuzzable=False)
        s_string("g", name='Request-URIh')
	...
	s_string("192.168.1.219", name="Host-Value", fuzzable = False)
	s_delim("\r\n", name="return-2", fuzzable = False)
	s_string("User-Agent:", name="User-Agent", fuzzable = False)
	s_delim(" ", name="space-4", fuzzable = False)
	s_string("Mozilla/5.0 (X11; Linux i686; rv:68.0) Gecko/20100101 Firefox/68.0", name="User-Agent-Value", fuzzable = False)
